...
| Icon registry (web)    | `apps/web/src/components/ui/Icon/Icon.constant.ts`                      | `.constant.ts`                | ICON_PATHS + IconNameType + default size / stroke width; extended with chevron-left and eye-off |
...
| Shared UI kit          | `apps/mobile/src/components/ui`                                               | `.component.tsx`              | Reusable primitives (`Button`, `Chip`, `AsyncBoundary`, `Toast`, `Modal`, `ConfirmDialog`, `EmptyState`, `ErrorState`, `LoadingSkeleton`, `StatusBadge`, `Icon`, `IconButton`, `Avatar`, `SearchBar`, `SegmentedControl`, `Toggle`, `GlassCard`, `ScreenHeader`, `SettingRow`, `ListRow`, `NavBar`, `AuthShell`, `AuthField`, `SocialAuthButton`) |
| Button (mobile)        | `apps/mobile/src/components/ui/Button/Button.component.tsx`                   | `.component.tsx`              | NativeWind Pressable button; primary/secondary/ghost variants; sm/md/lg sizes; loading state via ActivityIndicator |
| Button constants (mobile) | `apps/mobile/src/components/ui/Button/Button.constant.ts`                  | `.constant.ts`                | PRESS_OPACITY, size/variant union types |
| Button styles (mobile) | `apps/mobile/src/components/ui/Button/Button.styles.ts`                       | `.styles.ts`                  | NativeWind class-string maps for base, size, variant, text |
| Chip (mobile)          | `apps/mobile/src/components/ui/Chip/Chip.component.tsx`                       | `.component.tsx`              | NativeWind pill chip; neutral/accent/success/glass variants; optional dot indicator |
| Chip constants (mobile) | `apps/mobile/src/components/ui/Chip/Chip.constant.ts`                       | `.constant.ts`                | Chip variant union type; dot size class |
| Chip styles (mobile)   | `apps/mobile/src/components/ui/Chip/Chip.styles.ts`                           | `.styles.ts`                  | NativeWind class-string maps for container, text, variant |
...
| Auth field constants (mobile) | `apps/mobile/src/components/ui/AuthField/AuthField.constant.ts`       | `.constant.ts`                | AUTH_FIELD_ICON_SIZE dimension constant |
